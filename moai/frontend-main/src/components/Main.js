...
const Main = (props) => {
  ...
  useEffect(() => {
    const getMusicListDataFromServer = async () => {
      ...
      const responseDataHashTagFilter = responseDataHashTag.filter(
        (el) => el.length
      );
    ...
    <>
      ...
      <div className={classes.Main}>
        ...
        <div className={classes.MainImgAndTag}>
          ...
          <ul className={classes.MainImgAndTag__flex}>
            {mainPageData.hashTag?.map((el, idx) => (
              <li
                className={classes.MainImgAndTag__tag}
                key={idx}
              >{`#${el}`}</li>
            ))}
          </ul>
          ...
          <div className={classes.ProfileImgAndTitle}>
            <img
              src={`${mainPageData.composerThumbnailUrl}`}
              alt="메인페이지 사람 이미지"
            />
        ...
        <div className={classes.MusicTitleAndDescription}>
          ...
          <div>
            <h3 className={classes.MusicTitleAndDescription__title}>
              {mainPageData.musicIntro}
            </h3>
            <p className={classes.MusicTitleAndDescription__description}>
              언덕에 올라가는 느낌을 비트로 나타내어 보았습니다. 소중한 곡이라서
              가지고 있었는데, 더 이상 썩혀두기 아까워서 올려봅니다.곡의
              리듬감을 잘 살려줄 목소리를 찾아요.
            </p>
          </div>
        </div>
